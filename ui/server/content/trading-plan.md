...
> Сохранено: 2026-06-09 · Обновлено: 2026-06-15 · Категория: Финансы / Трейдинг
...
5. **Каждая закрытая сделка — постмортем.** Теперь генерится **сам**: при подтверждении карточки `⛔️ Закрыть` (шаг 3) и при карточке `📍 Закрытие обнаружено` (закрыл вне системы — шаг 3). Шаг 7 — только ручной fallback. Без журнала система не учится.
...
|--------------| ------------------------------------------------ | ----------------------- | -------------------------------------------------------- |
| каждые 5 мин | Автопилот сам выбирает шаг                       | 🤖 авто                 | лог + Telegram                                           |
...
| по событию   | **Шаг 7.** Закрыл сделку → журнал                | 🤖 авто (карточкой) / 👤 fallback | постмортем (генерится сам при закрытии через карточку)   |
...
**Тезис ⇄ ордер — направление и ручной гейт (чтобы не путать).** Карточки и кнопки Шага 2
**не создают** тезисы — наоборот, готовый тезис превращается в ордер. Тезис рождается накануне
вечером (Шаг 4, п. 5.5: `ingest`) в статусе `IDEA` и уже с уровнями (вход/стоп/цель/размер); его
`thesis_id` вшивается в строку watchlist, так что кандидат и тезис — **один объект**.

> ⚠️ **Предусловие:** карточки и кнопка «Поставить IB-bracket» срабатывают только для тезисов в
> статусе `ENTRY_READY`, а `ingest` оставляет их в `IDEA`. Автопромоушна `IDEA → ENTRY_READY` нет —
> это твоё решение «беру в работу». Пока не повысишь — премаркет ничего не пришлёт (`нет
> ENTRY_READY-кандидатов → нечего слать`).

Повысить отобранные кандидаты (или кнопкой перехода в карточке **Trader Memory** в дашборде):

```bash
python3 skills/trader-memory-core/scripts/trader_memory_cli.py store transition <thesis_id> ENTRY_READY --reason "беру в работу"
```

Только после этого по кандидату придёт карточка / появится кнопка bracket. Само нажатие ставит
ордер, но **тезис остаётся `ENTRY_READY`** и переходит в `ACTIVE`, лишь когда вход реально
исполнится (Шаг 3). Кратко: `ingest → IDEA` → 👤 `transition → ENTRY_READY` → карточка/кнопка →
`bracket` → исполнение → `ACTIVE`.

### 2.0. Автоматический режим — подтверждение через Telegram (рекомендуется)

Премаркет-слот (15:00) сам присылает в Telegram по карточке на каждого кандидата watchlist в
состоянии `ENTRY_READY` — с компанией, входом (buy/sell-stop), стопом, целью, кол-вом акций и
риском — и двумя кнопками: **✅ Открыть** / **✋ Не открывать**. **Лонг**-карточки идут при гейте
`allow`, **шорт**-карточки — при `restrict`/`cash-priority` (сторона фильтруется по гейту, как у
интрадей-сигналов).

- **✅ Открыть** → демон ставит в Interactive Brokers нативный bracket (вход + стоп + цель,
  `cOID`/`parentId`; для шорта — sell-short зеркально). Тезис **остаётся `ENTRY_READY`** и переходит
  в `ACTIVE` (с фактической ценой) **только когда стоп-вход реально исполнится** — это ловит
  интрадей-фолоуап автоматически.
- **✋ Не открывать** / отсутствие нажатия / таймаут окна демона → тезис остаётся `ENTRY_READY`,
  ордер не ставится, кнопки снимаются.
...
> Один и тот же демон обрабатывает **все** типы карточек: открытие (лонг/шорт), +2R (шаг 3),
> выходы по правилам (`⛔️ Закрыть`, шаг 3) и обнаруженные закрытия вне системы (`📍 Закрытие
> обнаружено`, шаг 3). +2R/close-карточки рассылает интрадей- и вечерний прогон, не премаркет.

### 2.0-bis. Из веб-дашборда (кнопки) — без Telegram

Тот же bracket можно поставить прямо из дашборда — это альтернативная поверхность
подтверждения над тем же реестром и теми же IB-хелперами (`place_ib_bracket`), без бота.

- В карточке **Trader Memory** открой тезис в состоянии `ENTRY_READY` → блок **«IB-ордера»** →
  **«Поставить IB-bracket»**. Уровни берутся из тезиса (`entry.target_price` / `exit.stop_loss` /
  `exit.take_profit{,_2,_3}` / `position.shares`), любой можно переопределить в форме. Сервер сам
  перечитывает тезис и **требует `ENTRY_READY`** (статус из запроса не доверяется). Если заданы
  **обе** цели T2 и T3 — объём дробится 50/25/25 на **N независимых нативных суб-брекетов**
  (по одному POST на транш, у каждого свой стоп/цель и свой cOID `{coid}-t{i}`). Так сделано
  намеренно: один POST с несколькими OCA-группами IB схлопывает в одну и оставляет «Pending
  Submit» — раздельные суб-брекеты вооружаются и исполняются каждый сам по себе.
- **«Отменить IB-bracket»** — снимает выставленный, но ещё не исполненный bracket. После
  исполнения входа отмена запрещена — закрывай через **«Закрыть»**.
- Вкладка **«Счёт IB»** → **«⟳ Сверить с тезисами»** — ловит исполнения и переводит тезис
  `ENTRY_READY → ACTIVE` (по фактической цене); TP/SL закрывают. Это та же синхронизация, что
  делает демон каждые 30с — кнопка нужна, когда демон не запущен. (Можно повесить на таймер раз
  в минуту.)

**Тот же двойной замок:** реальный ордер только при `IB_ALLOW_ORDER_PLACEMENT=true` в `.env`
(кнопка всегда шлёт `--live`); иначе — preview без отправки, тезис не меняется. `IB_PAPER_TRADING`
выбирает paper/live-счёт. Под капотом дашборд шеллит `bash scripts/run_watchlist_orders.sh
open-now|cancel|sync` (тот же лаунчер, что и для демона).
...
| 💰 **+2R** | Приходит **карточкой подтверждения** (`💰 Зафиксировать 50%` / `✋ Не сейчас`): по нажатию демон продаёт 50% рыночным + переносит стоп остатка в безубыток (и пишет trim в журнал). Без демона/`--live` — preview |
| ⚠️ **У СТОПА** (цена в 1% от стопа) | Ничего не двигать; просто будь рядом |
| ⛔️ **СТОП задет** | Проверить, что стоп-ордер исполнился. Если позиция ушла из IB — придёт карточка `📍 Закрытие обнаружено` (запись + постмортем по нажатию); иначе ручной шаг 7 |
...
> Тикеры с уже выставленным (через карточку) bracket-ордером **не дублируются** сигналом ОТКРОЙ.

Правила ведения позиции (на дневном закрытии) проверяет премаркет- и вечерний прогон.
Actionable-выходы приходят **карточкой подтверждения** (`⛔️ Закрыть` / `✋ Оставить`): по нажатию
демон закрывает позицию рыночным + снимает защитные ноги bracket + пишет `close` в журнал (CLOSED)
**и сразу генерит постмортем** (`pm_<thesis_id>.md` в `trading-data/journal/postmortems/`). Шаг 7
руками больше не нужен. 🛎-сводка в дайджесте остаётся для контекста.

| Событие | Действие |
|---|---|
| Дневное закрытие **ниже EMA20(≈21)** | Карточка `⛔️ Закрыть` → выйти из остатка |
| Позиция > 4 недель и закрытие ниже SMA50 | Карточка `⛔️ Закрыть` → трейл-выход |
| 15 торговых дней без +1R (шорт: 10) | Карточка `⛔️ Закрыть` (тайм-стоп); предупреждение «через N т.д.» приходит за 2 т.д. без кнопок |

**Закрытие вне системы (страховка).** Если позицию закрыл руками в TWS, или стоп исполнился, пока
ты не видел сигнала, — автопилот это заметит: каждые 15 мин (плюс премаркет/вечер) он сверяет
открытые тезисы с живым снимком позиций IB. Пропавшую позицию он не закрывает молча, а присылает
карточку `📍 Закрытие обнаружено` (`✅ Записать закрытие` / `✋ Не сейчас`). По нажатию — **только**
запись `close` в журнал + постмортем (ордер НЕ выставляется, позиция уже флэт). Цена выхода
подставляется приблизительная (последний филл IB) — поправь в журнале при необходимости. Если
снимок IB недоступен/не авторизован, карточка не шлётся (сбой Gateway не трактуется как «всё
закрыто»).
...
- **`allow` (лонг-ветка):** vcp-screener → heat → breakout-trade-planner (вход/стоп/цель/количество, earnings-гейт + фундаментальный флор) → claude проверяет топ-3 по графикам через `technical-analyst` (отклоняет слабые **и задаёт точные уровни входа/стопа/цели — они авторитетнее механических из планнера**, размер пересчитывается от риск-профиля) → полный `ticker-analysis` (новости + фундаментал) по **одной** лучшей бумаге, не разбиравшейся последние 5 т.д. → watchlist на завтра + регистрация тезисов в журнале. Это автоматизированные шаги 5.1–5.5.
...
> - **5.3** — явного распечатки чек-листа нет, но все 8 условий механически применяются: VCP-скринер фильтрует по SMA50/200/RS/ликвидности, breakout-trade-planner блокирует по earnings-гейту и фундаментальному флору (EPS/выручка), heat-check блокирует по позициям. Ручная проверка — перестраховка или нестандартный кандидат.
> - **5.4** — breakout-trade-planner запускается полностью (вход/стоп/цель/количество, earnings-gate + фундаментальный флор).
> - **5.5** — thesis-ingest запускается только для тикеров, прошедших chart-validation (не для всех ~10 VCP-кандидатов); уровни из breakout-trade-planner (pivot/stop/цель/количество) автоматически записываются в тезис; `thesis_id` инжектируется в watchlist и придёт в Telegram при ОТКРОЙ-сигнале. **Тезис создаётся в статусе `IDEA`** — чтобы утром по нему пришли карточки/кнопка Шага 2, его нужно вручную повысить до `ENTRY_READY` (автопромоушна нет; см. Шаг 2). Алерты TV ставятся сами.
>
...
**5.3. Чек-лист «беру / не беру»** — все 8 пунктов = ДА, иначе пропуск:
...
8. Фундаментальный флор: последний квартал EPS ≥ 0 и не падают одновременно EPS и выручка (YoY) — планировщик проверяет сам и блокирует (CANSLIM C/A через TradingView, без ключа)
...
## ШАГ 7 — Закрыл сделку → журнал (по событию, ручной fallback)

> **В большинстве случаев руками тут делать нечего.** Закрытие через карточку `⛔️ Закрыть` (шаг 3)
> и карточку `📍 Закрытие обнаружено` (закрытие вне системы) уже пишут `close` в журнал и генерят
> постмортем сами. Этот шаг нужен, только если обе карточки прошли мимо (демон не работал, Gateway
> был недоступен) или закрытие вообще не отслеживалось системой.
...
- **15:30–22:00** торгового дня → **интрадей-мониторинг** (каждые 15 мин): цены watchlist и открытых позиций без TradingView Desktop и ключей → сигналы ОТКРОЙ/ЗАКРОЙ/+2R в Telegram (каждый — один раз в день). Заодно сверяет открытые тезисы с живым снимком позиций IB и шлёт карточку `📍 Закрытие обнаружено` по тем, что пропали (закрытие вне системы)
...
| Сработал стоп — «обидно» | Это система работает. Закрытие запишется карточкой `📍 Закрытие обнаружено`; следующий кандидат |
| Карточка `📍 Закрытие обнаружено`, а позиция на месте | Редкий глюк снимка IB. Нажми `✋ Не сейчас` — ничего не запишется; карточка не повторится сегодня |
