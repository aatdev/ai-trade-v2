...
Re-verified 2026-06-17 (adding bottom-flow-divergence-screener): the "rerender-owned" loop below is a complete catch-all — it also corrects a page first written by `--skill <name>` (wrong nav 11), so you need not redo it via the no-flag run; just run rerender-owned over all dirs and `--check` goes green (18 pages rewritten that time = the shifted alphabetically-later ones).
