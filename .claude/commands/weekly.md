---
description: Запустить weekly-слот торгового планировщика
argument-hint: [--dry-run] [--force] [--no-telegram] [--date YYYY-MM-DD]
allowed-tools: Bash(bash scripts/run_trading_schedule.sh:*)
---
Запусти **weekly**-слот торгового планировщика:

`bash scripts/run_trading_schedule.sh --slot weekly $ARGUMENTS`

- Календарный гейт: слот рассчитан на **субботу** — в остальные дни сам пропустится (`rc=0`). Чтобы прогнать в другой день — добавь `--force`. Для безопасного теста — `--dry-run` (не вызывает claude/Telegram).
