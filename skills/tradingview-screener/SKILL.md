...
### Recipe 1: Value Dividend Screen (Kanchi-style)

```bash
python3 scripts/run_tv_screener.py \
  --filters "mkt_cap>2B,div_yield=3..8,pe<20,payout_ratio<60,div_growth_years>=5" \
  --columns dividends --sort=-div_yield --output-dir reports/
```

### Recipe 2: Minervini-style Trend Template

```bash
python3 scripts/run_tv_screener.py \
  --filters "close>SMA50,close>SMA200,SMA50>SMA200,perf_6m>0,mkt_cap>2B,avg_volume>300K" \
  --columns performance --sort=-Perf.3M --output-dir reports/
```

### Recipe 3: Oversold Quality (mean reversion)

```bash
python3 scripts/run_tv_screener.py \
  --filters "rsi<30,close>SMA200,roe>15,mkt_cap>10B" \
  --columns technicals --sort=rsi:asc --output-dir reports/
```

### Recipe 4: S&P 500 Sector Income

```bash
python3 scripts/run_tv_screener.py \
  --index sp500 --sectors "Finance,Utilities" --filters "div_yield>4" \
  --columns dividends --sort=-div_yield --output-dir reports/
```

### Recipe 5: High-growth Momentum

```bash
python3 scripts/run_tv_screener.py \
  --filters "revenue_growth>20,eps_growth>25,perf_3m>10,rel_volume>1,mkt_cap>2B" \
  --columns overview --sort=-perf_3m --output-dir reports/
```

### Recipe 6: Bullish Reversal Candles on Liquid Names
...
1. Start broad (3–4 filters) → check `Total matches`
2. >100 matches: tighten (add quality/liquidity filters); <5: relax
3. Switch column presets to inspect different dimensions
4. 0 matches with raw field names usually means a typo — the scanner
   silently ignores unknown filter fields (the script warns on 0 matches)

---

## Resources

- `references/tradingview_screener_filters.md` — full 238-filter catalog
  (8 UI categories → scanner fields), operations, enums (sectors, indexes,
  ratings, candlestick patterns), timeframe suffixes, All Stocks universe
  definition
- `scripts/run_tv_screener.py` — scanner API client and report generator
  (offline `--dry-run` mode for testing)
