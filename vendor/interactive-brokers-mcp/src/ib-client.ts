...
import { rejectUnauthorizedFor } from "./utils/tls-utils.js";
...
export class IBClient {
  ...
  private initializeClient(): void {
    ...
    this.client = axios.create({
      ...
      timeout: 30000,
      // Allow self-signed certificates only when talking to a local Gateway.
      httpsAgent: new https.Agent({
        rejectUnauthorized: rejectUnauthorizedFor(this.config.host),
      }),
    });

    // Add request interceptor to ensure authentication and log requests.
    // SECURITY: log method + URL only. Never log request headers (they carry the
    // IBKR session Cookie) or bodies (order payloads) to the on-disk log file.
  ...
  async initializeBrokerageSession(): Promise<boolean> {
    ...
    const noCookieClient = this.sessionCookieHeader
      ? axios.create({
          baseURL: this.baseUrl,
          timeout: 30000,
          httpsAgent: new https.Agent({ rejectUnauthorized: rejectUnauthorizedFor(this.config.host) }),
        })
      : undefined;
