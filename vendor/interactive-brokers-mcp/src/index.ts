...
function IBMCP({ config: userConfig }: { config: z.infer<typeof configSchema> }) {
  ...
  // SECURITY: the Flex token grants read access to account statements; never
  // log it. Username is also redacted to keep credentials out of the log file.
  if (logConfig.IB_FLEX_TOKEN) logConfig.IB_FLEX_TOKEN = '[REDACTED]';
  if (logConfig.IB_USERNAME) logConfig.IB_USERNAME = '[REDACTED]';
...
if (isMainModule) {
  ...
  if (logEnvConfig.IB_USERNAME) logEnvConfig.IB_USERNAME = '[REDACTED]';
  ...
  if (logFinalConfig.IB_USERNAME) logFinalConfig.IB_USERNAME = '[REDACTED]';
  ...
  if (logFinalConfigAfter.IB_USERNAME) logFinalConfigAfter.IB_USERNAME = '[REDACTED]';
