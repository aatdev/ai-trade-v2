...
export class IBGatewayManager {
  ...
  private async ensureRuntimeAvailable(platform: string, runtimePath: string): Promise<void> {
    ...
    // SECURITY: do not silently download + execute a JRE binary. The download has
    // no checksum/signature verification, so require an explicit opt-in and tell
    // the operator to verify integrity themselves.
    if (process.env.IB_ALLOW_RUNTIME_DOWNLOAD !== "true") {
      throw new Error(
        `Bundled runtime missing for ${platform} and automatic download is disabled. ` +
          `Pre-install a JRE under runtime/${platform}, or set IB_ALLOW_RUNTIME_DOWNLOAD=true to ` +
          `allow downloading a public musl JRE from ${runtimeUrl} (verify its integrity against ` +
          `BellSoft's published checksums before trusting it).`,
      );
    }

    this.log(
      `⚠️ IB_ALLOW_RUNTIME_DOWNLOAD=true: downloading a public musl JRE for ${platform} from ${runtimeUrl}. ` +
        `This download is NOT checksum-verified — confirm it matches BellSoft's published hash.`,
    );
