...
| **VCP Screener** | Mark MinerviniのVolatility Contraction Pattern を検出。Stage 2上昇トレンド銘柄のボラティリティ収縮とブレイクアウトポイントを識別。OHLCV は共有 TradingView データレイヤー経由 | <span class="badge badge-free">API不要</span> |
| **Swing Short Screener** | VCP Screener のショート版ミラー。5ファクター加重スコア（トレンド構造・相対力・出来高を伴うサポート割れ・切り下げ高値・流動性/借株適性）で Stage 4 下落トレンドの弱さを検出し、A/B/C/D グレードを付与。検出のみ（発注しない）。OHLCV は共有 TradingView データレイヤー経由 | <span class="badge badge-free">API不要</span> |
| **Bottom Flow Divergence Screener** | 52週安値圏まで売られた銘柄のうち、フロー（ファンダメンタル：TTM売上高がプラス成長＋営業CFプラス＝HOOD型／資金：Chaikin・マネーフロー＝逆張りのMRNA型）が安値を確認しない「底値ダイバージェンス」を検出。A / B-accum / B-fund グレードと turning/recovering/survivability タグを付与。検出のみ。scanner.tradingview.com（APIキー不要） | <span class="badge badge-free">APIキー不要</span> |
...
| **TradingView Screener** | 自然言語（英/露/日）をTradingViewスキャナーAPIクエリに変換 — TVの「All stocks」スクリーナータブと同じエンドポイント。8カテゴリ238フィルター（ファンダメンタル、テクニカル、配当、ローソク足パターン）、フィールド間比較（`close>EMA200`）、指数/セクター/レーティング選択に対応。結果はmarkdown + JSONレポート | <span class="badge badge-free">APIキー不要</span> |
| **Value Dividend Screener** | 高配当バリュー株をスクリーニング。P/E、P/B、配当利回り、3年成長トレンドで多段階フィルタリング。TradingViewデータレイヤー（APIキー不要）、任意のFINVIZプレスクリーンでユニバース拡大 | <span class="badge badge-free">APIキー不要</span> <span class="badge badge-optional">FINVIZ任意</span> |
| **Dividend Growth Pullback Screener** | 年間配当成長12%以上の高品質配当成長株で、RSI 40以下のプルバック中の銘柄を検出。TradingViewデータレイヤー（APIキー不要）、任意のFINVIZプレスクリーンでユニバース拡大 | <span class="badge badge-free">APIキー不要</span> <span class="badge badge-optional">FINVIZ任意</span> |
...
| **FTD Detector** | William O'Neilの手法でFollow-Through Day シグナルを検出。市場底打ち確認のためのデュアルインデックス追跡。OHLCV は共有 TradingView データレイヤー経由 | <span class="badge badge-free">API不要</span> |
...
| **[Market News Analyst]({{ '/ja/skills/market-news-analyst/' | relative_url }})** | WebSearch/WebFetchで過去10日間のニュースを収集。TradingView Desktop起動時はライブのTradingViewニュースフロー（ヘッドライン＋記事本文、銘柄/国フィルタ対応）も併用。定量的インパクトスコアリングでランキング | <span class="badge badge-free">API不要</span> |
...
| **Macro Regime Detector** | クロスアセット比率分析で構造的マクロレジーム転換（1-2年ホライズン）を検出。データは共有 TradingView レイヤー経由 | <span class="badge badge-free">API不要</span> |
...
| **[IBD Distribution Day Monitor]({{ '/ja/skills/ibd-distribution-day-monitor/' | relative_url }})** | QQQ/SPYのIBD式Distribution Dayを日次検出。25セッション失効・5%上昇無効化を追跡し、d5/d15/d25クラスタからNORMAL/CAUTION/HIGH/SEVERE判定とTQQQ/QQQエクスポージャ推奨を生成。OHLCV は共有 TradingView データレイヤー経由 | <span class="badge badge-free">API不要</span> |
...
| **[IB Portfolio Manager]({{ '/ja/skills/ib-portfolio-manager/' | relative_url }})** | Interactive Brokers 口座向けの同等分析。`interactive-brokers-mcp`（IB Gateway 同梱）経由。マルチアセット／マルチ通貨対応、分析専用（read-only）。Portfolio Manager の IB 版 | <span class="badge badge-api">IBKR必須</span> |
| **[Trader Memory Core]({{ '/ja/skills/trader-memory-core/' | relative_url }})** | 投資仮説のライフサイクルを永続追跡。スクリーナー出力をIDEAとして登録し、ENTRY_READY→ACTIVE→CLOSEDのステート遷移、ポジションサイジング付与、レビュースケジュール、MAE/MFE付きポストモーテム生成をサポート（価格は TradingView データレイヤー経由） | <span class="badge badge-free">API不要</span> |
...
| **[Breakout Trade Planner]({{ '/ja/skills/breakout-trade-planner/' | relative_url }})** | VCPスクリーナー出力からミネルヴィニ式ブレイクアウトトレードプランを生成。worst-case entryベースのGate、Alpaca と Interactive Brokers 向けの bracket 注文テンプレート（`--broker`; pre_place / post_confirm）、ポートフォリオヒート管理、決算日ゲート（公開 TradingView スキャナー）とタイムストップ注記（任意） | <span class="badge badge-free">API不要</span> |
...
- **[IB Portfolio Manager]({{ '/ja/skills/ib-portfolio-manager/' | relative_url }})** - Interactive Brokers 口座向けの同等分析（interactive-brokers-mcp 経由）
...
| Value Dividend Screener | - | 任意 | - |
| Dividend Growth Pullback Screener | - | 任意 | - |
