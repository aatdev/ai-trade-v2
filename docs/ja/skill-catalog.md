...
| **Swing Short Screener** | VCP Screener のショート版ミラー。5ファクター加重スコア（トレンド構造・相対力・出来高を伴うサポート割れ・切り下げ高値・流動性/借株適性）で Stage 4 下落トレンドの弱さを検出し、A/B/C/D グレードを付与。検出のみ（発注しない） | <span class="badge badge-api">FMP必須</span> |
...
| **[Market News Analyst]({{ '/ja/skills/market-news-analyst/' | relative_url }})** | WebSearch/WebFetchで過去10日間のニュースを収集。TradingView Desktop起動時はライブのTradingViewニュースフロー（キュレーション済みヘッドライン、銘柄/国フィルタ対応）も併用。定量的インパクトスコアリングでランキング | <span class="badge badge-free">API不要</span> |
