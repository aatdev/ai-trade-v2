...
> APIキー不要。OHLCV は共有 TradingView データレイヤー（`scripts/lib/tv_client.py` — vendored `tv` CLI / TradingView Desktop CDP + `state/metrics` キャッシュ）経由で取得します。`--api-key` / `FMP_API_KEY` は後方互換のため受け付けますが無視されます。
{: .tip }

| 項目 | 要否 | 説明 |
|------|------|------|
| TradingView Desktop (CDP) または新鮮な metrics キャッシュ | 必須 | ライブ株価データの取得 |
| Python 3.7+ | 必須 | スクリプト実行用 |
| APIキー | 不要 | 有料サブスクリプション一切不要 |
...
| オプション | 説明 | デフォルト |
|-----------|------|-----------|
| `--api-key` | 後方互換のみ（TradingView データレイヤーでは無視） | `$FMP_API_KEY` 環境変数 |
