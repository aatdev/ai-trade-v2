...
- 共有 TradingView データレイヤーによる MAE/MFE 算出（APIキー不要）
...
- **APIキー不要:** ポストモーテムの MAE/MFE は共有 TradingView データレイヤー（vendored `tv` CLI / metrics キャッシュ）で算出。コア機能（登録、遷移、決済、レビュー）は完全オフラインで動作
- **ステートディレクトリ:** `state/theses/` は初回使用時に自動作成

> MAE/MFE算出用の日次価格データは共有 TradingView データレイヤーから取得します（APIキー不要）。TVブリッジに接続できない場合でも、ポストモーテムは他のすべてのフィールドで生成されます（MAE/MFE は null）。
{: .tip }
...
P&L、保有日数、MAE/MFE（価格は TradingView データレイヤー経由、APIキー不要）を含むレポートが `state/journal/pm_{thesis_id}.md` に保存されます。

### ステップ 8: ポートフォリオヒート確認（ライブのオープンリスク）

新規エントリーを計画する前に、ACTIVE / PARTIALLY_CLOSED テーゼ全体のオープンリスクを集計します（`リスク = max(0, entry − exit.stop_loss) × 残株数`。ストップが建値以上ならヒートはゼロ）:
...
レポートにはヒート%（予算 `--max-portfolio-heat-pct`、デフォルト 6.0）、残りポジション枠、セクターエクスポージャーが含まれます。JSON のトップレベル（`open_risk_pct` / `sector_exposure`）は breakout-trade-planner の `--current-exposure-json` にそのまま渡せます。トレーリング時は `exit.stop_loss` を更新してください — ストップ未設定は `STOP_MISSING` として警告され `heat_complete: false` になります。
...
- **ポストモーテムは段階的に劣化。** MAE/MFE は TradingView データレイヤー経由。到達できない場合も P&L、保有日数、振り返りは完全オフラインで動作します。
...
- `skills/trader-memory-core/scripts/tv_price_adapter.py` -- MAE/MFE用 TradingView 日次終値アダプター（デフォルト、APIキー不要）
- `skills/trader-memory-core/scripts/fmp_price_adapter.py` -- レガシー FMP 価格アダプター（FMPキー保有者向けに残置）
