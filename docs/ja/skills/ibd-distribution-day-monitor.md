...
- **API キー:** 不要 — 日次 QQQ/SPY OHLCV は共有 TradingView データレイヤー（`scripts/lib/tv_client.py`）経由
- **Python 3.9+:** 標準ライブラリ + `requests`（既インストール）+ `pyyaml`（`pyproject.toml` に依存追加済み）
- **pandas 不要:** OHLCV はすべて `list[dict]` で処理（移植性と速度のため）

> API キーは不要です。`--api-key` / config の `data.api_key` / `FMP_API_KEY` は後方互換のため受け付けます（解決順: CLI > config > env）が、TradingView データレイヤーでは無視されます。
{: .tip }

> Distribution Day ルール自体は市場構造の前提に依存しないため、TradingView データレイヤーが提供する任意の流動性のある US ETF/指数で動作します。デフォルトは QQQ + SPY 向けに調整されています。
{: .note }
...
|   (TV/シンボル) |   |   prepare_effective_  |   |  pct_change <= -0.002 |
+-----------------+   |   history             |   |  かつ出来高増加       |
                      +-----------------------+   +-----------+-----------+
                                                              |
+-----------------+   +-----------------------+   +-----------v-----------+
...
1. **OHLCV 取得** — 各シンボルに対して `lookback_days + 5` の余裕を持たせて取得。50SMA フィルタが計算可能になるよう確保。データレイヤーは Issue #64 修正版で正しく truncate。
...
| `no_data_returned` | TradingView データレイヤーがいずれかのシンボルで空応答 |
...
### `FMP API key required` エラー（旧バージョンのみ）

**原因:** 旧バージョンはキー未設定で停止していました。

**対処:** 現行バージョンでは API キーは不要です（データは共有 TradingView レイヤー経由）。`--api-key` / `config.data.api_key` / `FMP_API_KEY` は後方互換のため受け付けますが無視されます。

### `as_of YYYY-MM-DD not found in loaded history`

**原因:** 指定した日付がデータ上の取引日ではない、または `lookback_days` の範囲外。
...
| `--api-key` | No | `FMP_API_KEY` 環境変数 | 後方互換のみ（TradingView レイヤーでは無視） |
