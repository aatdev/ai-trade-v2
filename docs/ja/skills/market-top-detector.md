...
4. [任意] VIX期間構造
   ...
   注: VIX/VIX3M クオートから（TradingView データレイヤー経由で）自動検出。
...
スクリプトは以下を実行します:
1. 共有 TradingView データレイヤーからS&P 500、QQQ、VIXのクオートと履歴を取得
...
- `skills/market-top-detector/scripts/fmp_client.py`（レガシー、未使用。データは `scripts/lib/tv_client.py` 経由）
