...
| TradingView News Flow | 任意 | TradingView Desktop起動＋TradingView MCP（`mcp__tradingview__*`、CDP `localhost:9222`）接続時、ライブのニュースフロータブをソースAとして読み取り、必要に応じて銘柄/国フィルタを適用。未接続時は自動スキップ。詳細はスキルの `references/tradingview_news_flow.md` |
...
ソースA（任意・利用可能なら優先）：TradingView Desktop起動時、ライブのニュースフロータブをTradingView MCPでスクレイプし、タイムスタンプ・配信元タグ付きのヘッドラインを取得（必要に応じ銘柄/国フィルタ適用）。ソースB（常時利用可）：以下6カテゴリで並行WebSearchを実行。両系統はイベント単位で重複排除し、信頼度ティアで優先順位付けします。
