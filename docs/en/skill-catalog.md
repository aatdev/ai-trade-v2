...
| **[Bottom Flow Divergence Screener]({{ '/en/skills/bottom-flow-divergence-screener/' | relative_url }})** | Finds beaten-down stocks near 52-week lows whose flows refuse to confirm the bottom — fundamental (TTM revenue still growing + positive operating cash flow, HOOD-type) and accumulation (Chaikin / Money Flow, contrarian MRNA-type) divergence. Grades A / B-accum / B-fund with turning/recovering/survivability tags. Detection-only; scanner.tradingview.com (no API key) | <span class="badge badge-free">No API Key</span> |
...
| **[Market News Analyst]({{ '/en/skills/market-news-analyst/' | relative_url }})** | Collects and analyzes market-moving news from the past 10 days via WebSearch, plus the live TradingView News Flow tab (headlines + full story bodies, instrument/country filters) when TradingView Desktop is running. Impact scoring: (Price Impact x Breadth) x Forward Significance | <span class="badge badge-free">No API</span> |
