...
| `evening-prep` | ~22:15 CET, trading days | Full regime via claude → gate. `allow`: vcp-screener → heat → breakout-trade-planner → claude chart validation → `schedule/watchlist_<date>.json` + thesis ingest. `restrict`/`cash-priority`: short branch (market pressure: top-risk ≥ 41 or DD ≥ 3, no fresh FTD) → swing-short-screener → short watchlist (1% risk sizing). Then TV alert sync: create `[WL]` Trigger/Stop/T1 alerts for candidates, drop stale ones (state: `logs/watchlist_alerts_state.json`). Requires TradingView Desktop (live data, no cache) — when it is down the slot notifies Telegram IMMEDIATELY and returns rc 1 (autopilot retries). Telegram summary (+ file, + alert counts). |
