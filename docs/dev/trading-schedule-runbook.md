...
| `evening-prep` | ~22:15 CET, trading days | Deterministic regime inputs first (market-breadth, uptrend, macro-regime, FTD, IBD distribution days → `market/`), then full regime via claude (market-top + news, told to reuse those inputs) → gate, then a deterministic exposure-coach re-run bounds it: the LLM may be stricter than the coach, never more permissive (decision clamped, ceiling ≤ coach); a failed/no-op claude step falls back to the coach's own read when breadth+uptrend are present. Gate files must be written by *this* run (mtime) and carry today's `date`. `allow`: vcp-screener → heat → breakout-trade-planner → claude chart validation (technical-analyst pass/reject **+ authoritative entry/stop/target levels** that override the planner's mechanical ones; size recomputed from the risk profile) → `schedule/watchlist_<date>.json` + thesis ingest → full `ticker-analysis` deep dive on the **single** best candidate not analyzed within `FRESH_ANALYSIS_WEEKDAYS` (reconcile levels / direction-flip; `AUTO_ANALYZE_TOP_N=1`). `restrict`/`cash-priority`: short branch **(only when `TRADING_ENABLE_SHORTS` is set — off by default → long-only, short branch skipped with a no-new-risk digest)**, market pressure: top-risk ≥ 41 or DD ≥ 3, a fresh (≤20h) ftd-detector report with no confirmed FTD → swing-short-screener → short watchlist (profile risk, sized at the worst fill). Only chart-validated candidates arm OPEN signals / order cards; a crashed screener or planner is reported and returns rc 1. Then TV alert sync: create `[WL]` Trigger/Stop/T1 alerts for candidates, drop stale ones (state: `logs/watchlist_alerts_state.json`). Requires TradingView Desktop (live data, no cache) — when it is down the slot notifies Telegram IMMEDIATELY and returns rc 1 (autopilot retries). Telegram summary (+ file, + alert counts). |
...
| `TRADING_ENABLE_SHORTS` | (unset → off) | Opt-in for shorting. Off by default the system is **long-only**: under a `restrict`/`cash-priority` gate the evening short branch is skipped and no `OPEN_SHORT` is armed. Set truthy (`1`/`true`/`yes`/`on`) to re-enable the short branch. Managing already-open short positions is never gated by this flag. |
